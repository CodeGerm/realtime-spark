...
export YARN_CONF_DIR=/app/cluster-config
export HADOOP_CONF_DIR=/app/cluster-config

cmd='$SPARK_HOME/bin/spark-submit --master yarn-client --class $MAIN --driver-memory $JOBSERVER_MEMORY
  --conf "spark.executor.extraJavaOptions=$LOGGING_OPTS"
  --driver-java-options "$GC_OPTS $JAVA_OPTS $LOGGING_OPTS $CONFIG_OVERRIDES"
  $appdir/spark-job-server.jar $1 $2 $conffile'
eval echo $cmd
eval $cmd >>/jobserver_manager.log 2>&1 &
